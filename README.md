...
* adapterBase source code (revision controlled)
* adapter example (not revision controlled)
* Java adapter library (revision controlled) *move to separate repository? If not, then it must share rev with adapterBase, per InterMet rules*
* adapter test program using the adapter example and adapter interface library (not revision controlled)
* lexicon specification (revision controlled using same revision as adapterBase)
